#'
#'@title Write model survey results to TCSAM2017 input file.
#'
#'@description Function to write model survey results to TCSAM2017 input file.
#'
...
#'@export
#'
writeSim.TCSAM2017.Surveys<-function(mc,mp,mr,fnSrvs,out.dir='.',showPlot=TRUE){
    #model dimensions
    d <- mc$dims;
    ...
    for (v in d$v$nms){
        ...
        cat("\n\n",file=conn)
        cat("#####################################################################\n",file=conn);
        cat("#TCSAM2017 Model File for",v,"\n",file=conn);
        cat("#####################################################################\n",file=conn);
